...
static BOOL validateProgram(GLuint prog)
{
    ...
    if (status == GL_FALSE) {
		HWLog(@"Failed to validate program %d", prog);
        return NO;
    }
...
@implementation KxMovieGLView {
    ...
    CGFloat         backgroudRed;
    CGFloat         backgroudGreen;
...
{
    [backgroundColor getRed:&backgroudRed green:&backgroudGreen blue:&backgroundBlue alpha:nil];
}
...
- (void)layoutSubviews
{
	...
	//modified by lvyi，由于opengles绘制的时候使用线程相关的环境变量，每次调用opengles
	//API获取参数或者绘制之前，需要将保存的环境变量设置进当前的线程
	[EAGLContext setCurrentContext: _context];
...
//    switch (self.displayRatio) {
//        case GLVIEW_DISPLAY_RATIO_DEFAULT:
//            height = _decoder.frameHeight;
//            break;
//        case GLVIEW_DISPLAY_RATIO_4_3:
//            height = (_decoder.frameWidth*3)/4;
//            break;
//        case GLVIEW_DISPLAY_RATIO_16_9:
//            height = (_decoder.frameWidth*9)/16;
//            break;
//    }
    
    if (self.displayRatio.x == self.displayRatio.y) {
        height = _decoder.frameHeight;
    }
    if (self.displayRatio.x != 0) {
        height = (_decoder.frameWidth*self.displayRatio.y)/self.displayRatio.x;
    }
    else
    {
        height = _decoder.frameHeight;
...
- (void)updateVertices
{
    ...
    const float width   = [self displayWidth];
    const float height  =  [self displayHeight];
...
//        return;
    ...
    glClearColor(backgroudRed, backgroudGreen, backgroundBlue, 1.0f);
