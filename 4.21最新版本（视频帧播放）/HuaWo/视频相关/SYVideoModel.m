...
+ (void)saveVideo:(SYVideoModel *)video{
    NSMutableArray *tempArray = [NSMutableArray arrayWithArray:[self videoListWithType:video.type]];
    ...
    [NSKeyedArchiver archiveRootObject:tempArray toFile:[self dataPathWithType:video.type]];
}
...
+ (NSArray *)videoListWithType:(SYVideoType)type {
    return [NSKeyedUnarchiver unarchiveObjectWithFile:[self dataPathWithType:type]];
}

+ (NSString *)dataPathWithType:(SYVideoType)type {
    return [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, true).firstObject stringByAppendingPathComponent:[NSString stringWithFormat:@"videoList%ld.data",(unsigned long)type]];
}
