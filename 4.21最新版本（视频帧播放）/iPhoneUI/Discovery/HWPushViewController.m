...
@interface HWPushViewController ()<UINavigationControllerDelegate, UITableViewDelegate, UITableViewDataSource, WMPlayerDelegate, findCommonCellDelegate>{
...
@implementation HWPushViewController
...
+(instancetype)pushViewController{

    return [[HWPushViewController alloc] initWithNibName:@"HWPushViewController" bundle:nil];
}
-(void)addMJRefresh{
    __weak __typeof(&*self)weakSelf = self;
    __unsafe_unretained UITableView *tableView = weakSelf.tableView;
    NSArray *para = @[@"qry_videolist", @"gentime", @"2"];
    // 下拉刷新
